...
int main() {
    ...
    int n, m, k;
    cin >> n >> m >> k;
    vector adj(n + 1, map<int, map<int, int>>());
    vector cnt(n + 1, map<int, int>());
    for (int i = 1; i <= m; i++) {
        int u, v, w;
        cin >> u >> v >> w;
        adj[u][v][w]++;
    ...
    Z ans = 0;
    for (int i = 1; i < k; i++) {
        if (not cnt[p[i]].count(p[i - 1])) {
            cout << "Stupid Msacywy!\n";
            return 0;
        }
        for (auto& [w, d] : adj[p[i]][p[i - 1]]) {
            debug(w, d);
            ans += Z(w) * d * power(Z(10), k - i - 1) / cnt[p[i]][p[i - 1]];
            debug(cnt[p[i]][p[i - 1]]);
        }
    }
    cout << ans << '\n';
